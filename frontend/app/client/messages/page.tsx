...
type Project = {
  id: string;
  assignments: { employee: ChatPartner }[];
};
          ...
          <div className="divide-y divide-slate-800">
            {(allPartners || []).map((p) => (
              <button
                ...
                className={`flex w-full flex-col px-4 py-3 text-left transition-all hover:bg-slate-800/50 ${selectedPartner?.id === p.id ? "bg-slate-800/80 ring-1 ring-sky-500/30" : ""
                  }`}
              >
        ...
        {selectedPartner ? (
          <div className="flex h-full flex-col space-y-4 min-h-0">
            <div className="flex items-center gap-3 md:gap-4 rounded-2xl border border-slate-800 bg-slate-900 px-4 md:px-6 py-4 shadow-xl shrink-0">
              ...
              <div className="h-10 w-10 shrink-0 rounded-full bg-gradient-to-br from-sky-500 to-indigo-600 flex items-center justify-center text-white font-bold shadow-lg">
              ...
              <div className="min-w-0">
                ...
                <div className="text-xs text-slate-500 font-medium uppercase tracking-tight truncate">
                  Direct Line — {selectedPartner.category}
                </div>
              </div>
            </div>
            ...
            <div className="text-center space-y-2">
              <div className="mx-auto w-12 h-12 rounded-2xl bg-slate-800/50 flex items-center justify-center text-slate-500">
                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="w-6 h-6">
                  <path strokeLinecap="round" strokeLinejoin="round" d="M20.25 8.511c.883.213 1.5.974 1.5 1.889v5.881c0 1.012-.924 1.766-1.892 1.584l-3.328-.623a.75.75 0 00-.542.121l-2.307 1.681a.75.75 0 01-1.121-.637v-1.13a.75.75 0 00-.75-.75H6.375c-.621 0-1.125-.504-1.125-1.125v-1.125m15-5.881c-.883-.213-1.5-.974-1.5-1.889V4.125c0-1.012.924-1.766 1.892-1.584l3.328.623a.75.75 0 01.542.474l.325 1.02a.75.75 0 00.125.228 12.016 12.016 0 011.667 6.012c0 2.213-.6 4.288-1.644 6.07a.75.75 0 01-.132.228l-.325 1.02a.75.75 0 01-.542.474l-3.328.623c-.968.182-1.892-.572-1.892-1.584v-2.5c0-.915.617-1.676 1.5-1.889Z" />
                </svg>
              </div>
